...
# Pull container images. It is important to use the long name with podman, not the short name, as podman sometimes has issues with docker image pulls.
...
# Pull AI models (replace 'examplemodel' with actual model names)
podman exec ollama ollama pull examplemodel
podman exec ollama ollama pull examplemodel:version    # Specific version
podman exec ollama ollama pull examplemodel:size      # Specific size (e.g. 7b, 13b)
...
# All the data from the conversations you had with the LLMs during that session is preserved on the SSD inside volumes. You can delete these if you wish for security purposes, or back them up somehow, whatever you like. I don't know the code for that, however.
...
# Pull new model. Substitute examplemodel with the filename from Ollama. It should include a semicolon ':' for example 'dolphin-llama3:70b' where '70b' is the size of the model.
podman exec ollama ollama pull examplemodel:latest
